...
def get_status(response):
    ...
    if response:
        try:
            ...
            logging.error('no status returned from Aria2 RPC server')
            return 'error'
    else:
...
class Aria2RpcServer(object):
    ...
    def __init__(
        ...
        ID:
          The ID to use in the Aria2JsonRpc returned by __enter__.
    ...
    def get_a2jr(self, new=False):
        ...
        if self.a2jr is None or new:
            ...
            self.a2jr = Aria2JsonRpc(self.ID, uri, **self.a2jr_kwargs)
        return self.a2jr
    ...
    def kill(self, nice=None):
        ...
        with self.lock:
            logging.debug('{}: attempting to stop PID {:d}'.format(
            ...
            for schwarzenegger in methods:
                ...
                if exit_code is not None:
                    logging.debug('{}: PID {:d} exit code: {:d}'.format(
                        self.__class__.__name__,
                        self.process.pid,
                        exit_code
                    ))
        ...
        logging.error('{}: failed to kill PID {:d}'.format(
            self.__class__.__name__,
            self.process.pid
        ))
        return False
    ...
    def launch(self):
        ...
        with self.lock:
            ...
            if launch:
                logging.debug('{}: no response to version request from server: {}'.format(
                    self.__class__.__name__,
                    self.cmd
                ))
                self.kill()
                logging.debug('{}: attempting to launch Aria2 RPC server: {}'.format(
                    self.__class__.__name__,
                    self.cmd
                ))
                if self.quiet:
...
class Aria2JsonRpc(object):
    ...
    def send_request(self, req_obj):
        ...
        logging.debug(json.dumps(req_obj, indent='  ', sort_keys=True))
        ...
        try:
            ...
            raise Aria2JsonRpcError(
                str(e),
                connection_error=(
                    '111' in str(e) or isinstance(e, ConnectionResetError)
                )
            )
        ...
        if results:
            ...
            for gid in gids:
                try:
                    ...
                    logging.error('Aria2 RPC server returned no status for GID {}'.format(gid))
                    yield 'error'
        else:
            logging.error('no response from Aria2 RPC server')
            for gid in gids:
                yield 'error'
